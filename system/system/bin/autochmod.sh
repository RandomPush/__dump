...
function wcnfirmwareupdate(){
    ...
    if [ -f ${saufwdir}/soc1_0_patch_mcu_1b_1_hdr.bin ]; then
       cp  ${saufwdir}/soc1_0_patch_mcu_1b_1_hdr.bin  ${pushfwdir}
       chown system:system ${pushfwdir}/soc1_0_patch_mcu_1b_1_hdr.bin
       chmod 666  ${pushfwdir}/soc1_0_patch_mcu_1b_1_hdr.bin
    fi

    if [ -f ${saufwdir}/soc1_0_ram_mcu_1b_1_hdr.bin ]; then
       cp  ${saufwdir}/soc1_0_ram_mcu_1b_1_hdr.bin  ${pushfwdir}
       chown system:system ${pushfwdir}/soc1_0_ram_mcu_1b_1_hdr.bin
       chmod 666  ${pushfwdir}/soc1_0_ram_mcu_1b_1_hdr.bin
    fi

    if [ -f ${saufwdir}/soc1_0_ram_bt_1b_1_hdr.bin ]; then
       cp  ${saufwdir}/soc1_0_ram_bt_1b_1_hdr.bin  ${pushfwdir}
       chown system:system ${pushfwdir}/soc1_0_ram_bt_1b_1_hdr.bin
       chmod 666 ${pushfwdir}/soc1_0_ram_bt_1b_1_hdr.bin
    fi

    if [ -f ${saufwdir}/soc1_0_ram_wifi_1b_1_hdr.bin ]; then
       cp  ${saufwdir}/soc1_0_ram_wifi_1b_1_hdr.bin  ${pushfwdir}
       chown system:system ${pushfwdir}/soc1_0_ram_wifi_1b_1_hdr.bin
       chmod 666 ${pushfwdir}/soc1_0_ram_wifi_1b_1_hdr.bin
    fi

    if [ -f ${saufwdir}/WIFI_RAM_CODE_soc1_0_1b_1.bin ]; then
       cp  ${saufwdir}/WIFI_RAM_CODE_soc1_0_1b_1.bin  ${pushfwdir}
       chown system:system ${pushfwdir}/WIFI_RAM_CODE_soc1_0_1b_1.bin
       chmod 666 ${pushfwdir}/WIFI_RAM_CODE_soc1_0_1b_1.bin
    fi
...
function oidtlogs() {
    ...
    log_path=`getprop sys.oppo.oidtlog.path`
    if [ "$log_path" ];then
        oidt_root=${log_path}
    else
    ...
    log_config_file=`getprop sys.oppo.oidtlog.config`
    echo "log config file: "${log_config_file}
    ...
    setprop sys.oppo.oidtlog.config ''
    setprop sys.oppo.oidtlog.path ''
...
function logStable(){
    ...
    cp -r data/oppo/log/DCS/de/AEE_DB/ ${oidt_root}/log/stable
    cp -r data/oppo/log/DCS/en/AEE_DB/ ${oidt_root}/log/stable
    cp -r data/vendor/aee_exp/ ${oidt_root}/log/stable
}

function logPerformance(){
    mkdir -p ${oidt_root}/log/performance
    cat /proc/meminfo > ${oidt_root}/log/performance/meminfo_fs.txt
    dumpsys -t 15 meminfo > ${oidt_root}/log/performance/meminfo_dump.txt
...
function logPower(){
    ...
    #sleep 3
    #endif /* COLOROS_EDIT */
    cp /data/oppo/psw/powermonitor_backup -r ${oidt_root}/log/power
